...
class LinkedList:
    ...
    def reverse(self):
        # If the list is empty or has one element, no need to reverse
        if self.head is None or self.head.next is None:
            return

        # create 2 variables to avoid losing track of the list
        prev = None
        current = self.head

    # Create a while loop to loop through the list
        while current is not None:
